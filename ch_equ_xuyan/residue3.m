function [ Residue3 ] = residue3( x,Nelm,Ord,uh,rh,ph,Time,period )
...
%compute the maximum of the abs(uh) 
amax=0;
for ne=1:Nelm
    xmid=0.5*(x(ne)+x(ne+1));
    umid=evalue_uh( Ord,Nelm,x,uh,xmid );
    if amax<=abs(umid)
        amax=abs(umid);
    end
end
...
%         pRm=2*(0.25*exp(-1*(XR-0.25*Time)))^2;
%     else
        uLp=uh(ne,:)*uf(:,1);                   
        uLm=uh(nei(ne,1),:)*uf(:,2);
        uRp=uh(nei(ne,2),:)*uf(:,1);          
...
%     end
    ...
    fhat_L=0.5*(3/2*uLm^2+3/2*uLp^2-3*amax*(uLp-uLm));
    fhat_R=0.5*(3/2*uRm^2+3/2*uRp^2-3*amax*(uRp-uRm));
